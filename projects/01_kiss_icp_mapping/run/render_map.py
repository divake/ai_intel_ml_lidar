...
# pack 3 int64 keys -> 1 via unique on rows
...
render("top", "map_topdown_intensity.png",
       f"lab_room1  top-down (intensity)  |  {world.shape[0]:,} pts @ {args.voxel} m voxel")
render("side", "map_side_intensity.png",
       f"lab_room1  side view (intensity)  |  height = up")
...
fig.update_layout(
    title=f"lab_room1 interactive ({len(sel):,} pts shown)",
